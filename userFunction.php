<?php
    ...
    while($data = $DB_site2->fetch_array($requete)) {
      ...
      $spy .= ""
        . "<tr class=\"td_alt" . $rowalt . "\">"
          . "<td>" . $data['site'] . "</td>"
          . "<td align=\"right\">" . number_format($data['price'], 2, '.',' ') . " &euro;</td>"
          . "<td align=\"right\" style=\"color:" . ($data['price'] < $article->prix ? "red" : "green") . ";\">"
              . "$pc"
          . "</td>"
    ...
    if($ret['spy_nb'] > 0) {
      if($ret['spy_prix'] < $prixtotal) {
        ...
        if($admin_salon == false) {
          $ret['rows']['Concurrents']	= $ret['spy_nb']
            . " "
            . "<a href=\"export/export_marge_article.php?artids=" . $artid . "&active[]=0&active[]=1&active[]=2&active[]=3\" target=\"_blank\">"
              . "Voir"
            . "</a>";
        }
      }
	...
	$nbfile = $DB_site2->query_first(""
    . "SELECT COUNT(*)" . " "
    . "FROM article_files" . " "
    . "WHERE artid=" . intval($article->artid)
  );
  ...
  if($nbfile[0] > 0) {
      ...
      . "<img "
          ...
          . "width=\"30\""
          . "class=\"picto\""
          . "onclick=\"fb.show('article_files.php?artid=" . $article->artid . "&parent=0')\""
      . ">";
	}
